...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
                 groups=1, width_per_group=64, norm_layer=None, affine=True):
    ...
    def forward(self, x):
        ...
        x = self.relu(x)
        x = self.maxpool(x)
        # print(f'input block1: ', x.shape)
        x = self.layer1(x)
        # print(f'output block1: ', x.shape)
        x = self.layer2(x)
        # print(f'output block2: ', x.shape)
        x = self.layer3(x)
        # print(f'output block3: ', x.shape)
        x = self.layer4(x)
        # print(f'output block4: ', x.shape)
...
if __name__ == "__main__":
    ...
    model = resnet50(num_classes = 200, affine=True)
    print(model)